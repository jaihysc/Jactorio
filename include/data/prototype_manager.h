...
namespace jactorio::data
{
	...
	public:
		...
		TProto* DataRawGet(const std::string& iname) const;
		...
		template <typename TProto,
		          std::enable_if_t<std::is_base_of_v<PrototypeBase, TProto>, int>  = 0>
		TProto* DataRawGet(DataCategory data_category, const std::string& iname) const;
		...
		template <typename TProto = PrototypeBase,
		          std::enable_if_t<std::is_base_of_v<PrototypeBase, TProto>, int>  = 0>
		J_NODISCARD TProto* FindProto(const std::string& iname) const;
	...
	template <typename T,
	          std::enable_if_t<std::is_base_of_v<PrototypeBase, T>, int>>
	T* PrototypeManager::DataRawGet(const std::string& iname) const {
	...
	template <typename T,
	          std::enable_if_t<std::is_base_of_v<PrototypeBase, T>, int>>
	T* PrototypeManager::DataRawGet(const DataCategory data_category, const std::string& iname) const {
	...
	template <typename T,
	          std::enable_if_t<std::is_base_of_v<PrototypeBase, T>, int>>
	std::vector<T*> PrototypeManager::DataRawGetAll(const DataCategory type) const {
	...
	template <typename T,
	          std::enable_if_t<std::is_base_of_v<PrototypeBase, T>, int>>
	std::vector<T*> PrototypeManager::DataRawGetAllSorted(const DataCategory type) const {
	...
	template <typename TProto, typename ... TArgs, std::enable_if_t<std::is_base_of_v<PrototypeBase, TProto>, int>>
	...
	TProto* PrototypeManager::FindProto(const std::string& iname) const {
