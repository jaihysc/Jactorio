...
namespace game
{
	...
	protected:
		...
		jactorio::game::TransportLineSegment* segment_ =
			new jactorio::game::TransportLineSegment(jactorio::data::Orientation::left,
	...
	TEST_F(TransportLineStructureTest, CanInsertFilledTransportLine) {
		...
		//	segment->append_item(true, kItemSpacing, item_proto.get());
		//	segment->append_item(true, kItemSpacing, item_proto.get());
		//	segment->append_item(true, kItemSpacing, item_proto.get());
		//	segment->append_item(true, kItemSpacing, item_proto.get());
	...
	TEST_F(TransportLineStructureTest, IsActive) {
		...
		EXPECT_FALSE(segment_->IsActiveLeft());
		EXPECT_FALSE(segment_->IsActiveRight());

		segment_->AppendItem(false, 0.f, itemProto_.get());
		segment_->AppendItem(true, 0.f, itemProto_.get());

		// Has items, now active
		EXPECT_TRUE(segment_->IsActiveLeft());
		EXPECT_TRUE(segment_->IsActiveRight());
	...
	TEST_F(TransportLineStructureTest, AppendItem) {
		auto line_segment = std::make_unique<jactorio::game::TransportLineSegment>(
	...
	TEST_F(TransportLineStructureTest, AppendItemFirstItem) {
		...
		EXPECT_FLOAT_EQ(line_segment.get()->left[1].first.getAsDouble(), jactorio::game::kItemSpacing);

		line_segment->AppendItem(true, 0, itemProto_.get());  // Not ok, offset changed to kItemSpacing
		EXPECT_FLOAT_EQ(line_segment.get()->left[2].first.getAsDouble(), jactorio::game::kItemSpacing);
	}
	...
	TEST_F(TransportLineStructureTest, TryInsertItem) {
		...
		EXPECT_EQ(line_segment->lIndex, 0);
	}
	...
	TEST_F(TransportLineStructureTest, BackItemDistanceLeft) {
		...
		EXPECT_FLOAT_EQ(line_segment->lBackItemDistance.getAsDouble(), 0.f);


		// Appending
		line_segment->AppendItem(true, 1.2, itemProto_.get());
		EXPECT_FLOAT_EQ(line_segment->lBackItemDistance.getAsDouble(), 1.2f);

		line_segment->AppendItem(true, 3, itemProto_.get());
		EXPECT_FLOAT_EQ(line_segment->lBackItemDistance.getAsDouble(), 4.2f);

		line_segment->AppendItem(true, 1.8, itemProto_.get());
		EXPECT_FLOAT_EQ(line_segment->lBackItemDistance.getAsDouble(), 6.f);

		// Inserting (Starting at 6.f)
		line_segment->InsertItem(true, 7, itemProto_.get());
		EXPECT_FLOAT_EQ(line_segment->lBackItemDistance.getAsDouble(), 7.f);

		line_segment->InsertItem(true, 2, itemProto_.get());
		EXPECT_FLOAT_EQ(line_segment->lBackItemDistance.getAsDouble(), 7.f);  // Unchanged

		EXPECT_FLOAT_EQ(line_segment->rBackItemDistance.getAsDouble(), 0.f);
	}
	...
	TEST_F(TransportLineStructureTest, BackItemDistanceRight) {
		...
		EXPECT_FLOAT_EQ(line_segment->rBackItemDistance.getAsDouble(), 1.2f);

		line_segment->AppendItem(false, 3, itemProto_.get());
		EXPECT_FLOAT_EQ(line_segment->rBackItemDistance.getAsDouble(), 4.2f);

		line_segment->AppendItem(false, 1.8, itemProto_.get());
		EXPECT_FLOAT_EQ(line_segment->rBackItemDistance.getAsDouble(), 6.f);

		// Inserting (Starting at 6.f)
		line_segment->InsertItem(false, 7, itemProto_.get());
		EXPECT_FLOAT_EQ(line_segment->rBackItemDistance.getAsDouble(), 7.f);

		line_segment->InsertItem(false, 2, itemProto_.get());
		EXPECT_FLOAT_EQ(line_segment->rBackItemDistance.getAsDouble(), 7.f);  // Unchanged
