...
namespace game::logic
{
	void generate_test_world() {
		...
		for (int y = 0; y < 32; ++y) {
			...
			for (int x = 0; x < 32; ++x) {
				...
				chunk_tiles[y * 32 + x]
					.set_layer_tile_prototype(Chunk_tile::chunk_layer::base, tile_ptr);
			}
			// printf("\n");
		}
	...
	TEST(placement_controller, place_entity_1x1_valid) {
		...
		EXPECT_EQ(
			chunk->tiles_ptr()[0].get_layer_entity_prototype(jactorio::game::Chunk_tile::chunk_layer::entity),
			entity.get());
	}
	...
	TEST(placement_controller, place_entity_3x3_valid) {
		...
		for (int y = 5; y < 5 + 3; ++y) {
			for (int x = 5; x < 5 + 3; ++x) {
				...
				EXPECT_EQ(
					chunk->tiles_ptr()[index].get_layer_entity_prototype(
	...
	TEST(placement_controller, remove_entity_3x3_valid_1) {
		...
		for (int y = 5; y < 5 + 3; ++y) {
			for (int x = 5; x < 5 + 3; ++x) {
				const auto index = y * 32 + x;
				EXPECT_EQ(chunk->tiles_ptr()[index].get_layer_entity_prototype(
					          jactorio::game::Chunk_tile::chunk_layer::entity),
				          nullptr);
