...
namespace game
{
	...
	protected:
		jactorio::game::PlayerData player_data_{};
		...
		void setup_test_recipe() {
			...
			DataRawAdd("item-product", item_product_);

			item1_ = new jactorio::data::Item();
			DataRawAdd("item-1", item1_);

			item2_ = new jactorio::data::Item();
			DataRawAdd("item-2", item2_);

			item_sub1_ = new jactorio::data::Item();
			DataRawAdd("item-sub-1", item_sub1_);

			item_sub2_ = new jactorio::data::Item();
			DataRawAdd("item-sub-2", item_sub2_);

			final_recipe_ = new jactorio::data::Recipe();
			final_recipe_->Set_ingredients({{"item-1", 3}, {"item-2", 1}});
			final_recipe_->SetProduct({"item-product", 1});

			DataRawAdd("item-product-recipe", final_recipe_);

			item_recipe_ = new jactorio::data::Recipe();
			item_recipe_->Set_ingredients({{"item-sub-1", 5}, {"item-sub-2", 10}});
			item_recipe_->SetProduct({"item-1", 2});

			DataRawAdd("item-1-recipe", item_recipe_);
		}
	};
	...
	TEST_F(PlayerDataRecipeTest, RecipeSelectRecipeGroup) {
		...
		player_data_.RecipeGroupSelect(1);

		EXPECT_EQ(player_data_.RecipeGroupGetSelected(), 1);
	}
	...
	TEST_F(PlayerDataRecipeTest, RecipeQueue) {
		...
		// Register items
		DataRawAdd("item-1", item);

		DataRawAdd("item-product", item_product);
		...
		player_data_.inventoryPlayer[0] = {item, 10};

		// Queue 2 crafts
		player_data_.RecipeQueue(&recipe);
		player_data_.RecipeQueue(&recipe);

		// Used up 2 * 2 (4) items
		EXPECT_EQ(player_data_.inventoryPlayer[0].first, item);
		EXPECT_EQ(player_data_.inventoryPlayer[0].second, 6);

		EXPECT_EQ(player_data_.inventoryPlayer[1].first, nullptr);
		...
		player_data_.RecipeCraftTick(30);  // Not done yet

		EXPECT_EQ(player_data_.inventoryPlayer[1].first, nullptr);

		player_data_.RecipeCraftTick(90);

		EXPECT_EQ(player_data_.inventoryPlayer[1].first, item_product);
		EXPECT_EQ(player_data_.inventoryPlayer[1].second, 2);
	}
	...
	TEST_F(PlayerDataRecipeTest, RecipeCraftResurse) {
		...
		player_data_.inventoryPlayer[0] = {item2_, 1};

		player_data_.inventoryPlayer[1] = {item_sub1_, 10};
		player_data_.inventoryPlayer[2] = {item_sub2_, 20};

		player_data_.RecipeCraftR(final_recipe_);

		player_data_.RecipeCraftTick(9999);  // Should be enough ticks to finish crafting

		EXPECT_EQ(player_data_.inventoryPlayer[0].first, item1_);  // 1 extra item 1 from crafting
		EXPECT_EQ(player_data_.inventoryPlayer[0].second, 1);

		EXPECT_EQ(player_data_.inventoryPlayer[1].first, item_product_);
		EXPECT_EQ(player_data_.inventoryPlayer[1].second, 1);

		EXPECT_EQ(player_data_.GetCraftingItemDeductions().size(), 0);
		EXPECT_EQ(player_data_.GetCraftingItemExtras().size(), 0);
	}
	...
	TEST_F(PlayerDataRecipeTest, RecipeCraftResurse2) {
		...
		player_data_.inventoryPlayer[0] = {item2_, 4};
		player_data_.inventoryPlayer[1] = {item_sub1_, 30};
		player_data_.inventoryPlayer[2] = {item_sub2_, 60};
		...
		EXPECT_EQ(player_data_.inventoryPlayer[0].first, item_product_);
		EXPECT_EQ(player_data_.inventoryPlayer[0].second, 4);

		EXPECT_EQ(player_data_.GetCraftingItemDeductions().size(), 0);
		EXPECT_EQ(player_data_.GetCraftingItemExtras().size(), 0);

		// Ensure there were no excess items
		for (int i = 1; i < jactorio::game::PlayerData::kInventorySize; ++i) {
			EXPECT_EQ(player_data_.inventoryPlayer[i].first, nullptr);
		}
	}
	...
	TEST_F(PlayerDataRecipeTest, RecipeCanCraft) {
		...
		// 3-B + 1-C -> 1-A
		// 5-B1 + 10-B2 -> 2-B
		player_data_.inventoryPlayer[0] = {item1_, 1};
		player_data_.inventoryPlayer[1] = {item2_, 2};

		player_data_.inventoryPlayer[2] = {item_sub1_, 5};
		player_data_.inventoryPlayer[3] = {item_sub2_, 10};

		// Crafting item-product:
		// Item 1 requires one batch to be crafted
		// Crafting requires for item 2 met
		EXPECT_EQ(player_data_.RecipeCanCraft(final_recipe_, 1), true);
	}
	...
	TEST_F(PlayerDataRecipeTest, RecipeCanCraftInvalid) {
		...
		// Item 1 requires one batch to be crafted
		// Crafting requires for item 2 met
		EXPECT_EQ(player_data_.RecipeCanCraft(final_recipe_, 1), false);
	}
	...
	TEST_F(PlayerDataRecipeTest, RecipeCanCraftInvalid2) {
		...
		player_data_.inventoryPlayer[1] = {item_sub1_, 10};
		player_data_.inventoryPlayer[2] = {item_sub2_, 5};
	...
	TEST_F(PlayerDataRecipeTest, RecipeCraftFullInventory) {
		...
		setup_test_recipe();
		player_data_.inventoryPlayer[0] = {item1_, 3};
		player_data_.inventoryPlayer[1] = {item2_, 1};

		EXPECT_TRUE(player_data_.RecipeCanCraft(final_recipe_, 1));
		player_data_.RecipeCraftR(final_recipe_);

		// Fill inventory so crafted item cannot be returned
		jactorio::data::Item filler_item{};
		for (auto& slot : player_data_.inventoryPlayer) {
			slot.first  = &filler_item;
			slot.second = 1;
		}

		// Will not return item until slot is freed
		player_data_.RecipeCraftTick(9999);
		player_data_.RecipeCraftTick(9999);
		player_data_.RecipeCraftTick(9999);

		player_data_.inventoryPlayer[0] = {nullptr, 0};
		player_data_.RecipeCraftTick(9999);
