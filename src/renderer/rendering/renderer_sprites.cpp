...
void jactorio::renderer::Renderer_sprites::set_image_positions(Image_position& image_position,
                                                               const sf::Vector2u image_dimensions,
                                                               unsigned int& offset_x) {

	image_position.top_left     = Position_pair{static_cast<float>(offset_x),     0};
	image_position.top_right    = Position_pair{static_cast<float>(offset_x + image_dimensions.x), 0};
	
	image_position.bottom_left  = 
		Position_pair{static_cast<float>(offset_x), static_cast<float>(image_dimensions.y)};
	image_position.bottom_right = 
		Position_pair{static_cast<float>(offset_x + image_dimensions.x), static_cast<float>(image_dimensions.y)};
...
jactorio::renderer::Renderer_sprites::Spritemap_data jactorio::renderer::Renderer_sprites::gen_spritemap(
	std::string* image_paths, const unsigned short count) {
	...
	auto* concat_image_old = new sf::Image{};  // This is a pointer to avoid copying when moving working_image into this
	concat_image_old->loadFromFile(
		core::File_system::resolve_path(image_paths[image_index++])
	);
	...
	while (image_index < count) {
		...
		const unsigned int pixels_y = (image_old_prop.y > image_new_prop.y) ? image_old_prop.y : image_new_prop.y;
