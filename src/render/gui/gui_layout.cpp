...
void render::DrawCursorTooltip(game::PlayerData& player_data,
    ...
    if (player_data.inventory.GetSelectedItem() != nullptr)
...
void render::DrawItemSlot(const MenuData& menu_data,
                          const uint8_t scale,
    ...
    // Visible button, lower width such that a border is visible between slots
    {
        ...
        if (sprite_iid == 0) {
            ...
            const unsigned int button_size = scale * kInventorySlotWidth +
